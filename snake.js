...
function getRandomFreeCell(map) {
    ...
    for (const cell of map.flat()) { // делает массив плоским
...
function drawGameMap(map) {
    for (const cell of map.flat()) {
        const param = {
            x: GAME_PADING + cell.x * (CELL_SIZE + CELL_MARGIN),
            y: GAME_PADING + cell.y * (CELL_SIZE + CELL_MARGIN),
            width: CELL_SIZE,
            height: CELL_SIZE,
...
function drawPause() {
    ...
    context.fillText(`Ваш счет: ${cooldown}`, canvas.width / 2, canvas.height / 2);
    context.fillText("Нажмите enter для продолжения", canvas.width / 2, canvas.height / 2 + 58);
}
